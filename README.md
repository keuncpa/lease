...
1. **계약서 AI 추출** — 규칙기반(키 불필요) / LLM(`/api/extract`) 동일 스키마. 필드별 `값+신뢰도+근거조항`.
2. **IFRS 16 계산 엔진** — 리스부채 PV·사용권자산·상각표·개시분개. 외부 의존성 0, 단위테스트 6종 통과.
3. **감사 검증** — 독립 재계산, 할인율 합리성·완전성·표시공시 이상탐지(심각도+권고절차).
4. **감사조서 내보내기** — 추적 가능한 워크페이퍼(.md).
...
**Node.js · TypeScript · Next.js 14 (App Router, React)** · Tailwind CSS · Node 내장 test runner · **Gen AI API(Anthropic) + 오픈소스 LLM(Ollama)** 멀티 프로바이더 추출
...
lib/
  ...
  extraction/ extractor.ts · schema.ts              ← 추출
